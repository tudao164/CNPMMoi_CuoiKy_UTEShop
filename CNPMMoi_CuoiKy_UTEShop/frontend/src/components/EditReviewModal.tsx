...
import type { UpdateReviewData, Review } from '@/types/review.types';
...
export default function EditReviewModal({
  ...
  useEffect(() => {
    if (review.images && review.images.length > 0) {
      const previews = review.images.map((img) => getImageUrl(img));
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">Chỉnh sửa đánh giá</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              💡 <strong>Lưu ý:</strong> Đánh giá sau khi chỉnh sửa sẽ cần được kiểm duyệt lại.
            </p>
          </div>
